...
final class CardDataService: ObservableObject {
    ...
    // MARK: - Dummy Data
    func insertDummyCards() {
        let dummyCards = createDummyCards()
    ...
    private func createDummyCards() -> [CardModel] {
        return [
            CardModel(
                id: UUID(),
                name: "Howard",
...
extension CardDataService {
    static func insertDummyCards(into context: ModelContext) {
        let service = CardDataService(context: context)
        service.insertDummyCards()
    }
}
